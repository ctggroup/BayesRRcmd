...
// Copied from Robert Eisele
// https://gist.github.com/infusion/43bd2aa421790d5b4582
void Data::readCSV(const string &bedFile) {

	std::ifstream in(bedFile.c_str());
...
/*	for(int i=0; i<numIncdSnps; i++){
		double mean =Z.col(i).array().sum()/numKeptInds;
		cout << Z.col(i).array().sum()/numKeptInds << endl;
		cout << ((Z.col(i).array() * Z.col(i).array()).sum() - numKeptInds* mean*mean) << endl;
		Z.col(i) = (Z.col(i).array() - mean) / ((Z.col(i).array() * Z.col(i).array()).sum() - numKeptInds* mean*mean);
	}*/
